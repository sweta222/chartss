import { scaleLinear, axisLeft, axisBottom, scaleBand, max, line, pie, arc } from 'd3';
...
const drawBar = arg => {
  ...
  const xScale = drawLinearAxis({
    domainMin: 0,
    domainMax: max(arg.userData.datasets, d => d.labely),
    rangeMin: 0,
    rangeMax: arg.width,
...
const drawPie = arg => {
  const piee = pie()(arg.userData.datasets.map(d => d.labely));
